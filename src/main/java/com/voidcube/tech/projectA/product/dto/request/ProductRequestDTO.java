...
public record ProductRequestDTO(
        ...
        @DecimalMin(
                value = "0.00",
                inclusive = true,
                message = "{validation.product.price.minimum}"
        )
        @Digits(
                integer = 17,
                fraction = 2,
                message = "{validation.product.price.digits}"
        )
        BigDecimal price,
        ...
        List<
                @NotBlank(message = "{validation.product.tag.name.required}")
                @Size(max = 255, message = "{validation.product.tag.name.size}")
                String
        > tags,
